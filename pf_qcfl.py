...
class qcfl_Inst(problem_functions.Pb_functions):
    ...
    def complete(self, env, gp_env) :
        ...
        # objective function
        obj_expr = gp.quicksum(env.cost1[i]*x[i] for i in range(env.r)) + gp.quicksum((gp.quicksum(env.cost2[s,i][j] * y[s][i,j] for i in range(env.r) for j in range(env.N))
                                                                                       + env.h * gp.quicksum(gp.quicksum(gp.quicksum(y[s][i,j]*y[s][i,l] for l in range(env.N) if l>j) for j in range(env.N)) for i in range(env.r)))*env.prob[s] for s in range(env.N_scen))
        ...
        for s in range(env.N_scen) :
            ...
            for i in range(env.r) :
                ...
                con_capacity_add[s,i] = model.addConstr(gp.quicksum(env.dem[s,i][j]*y[s][i,j] for j in range(env.N)) >= 0.80*env.V[i]*x[i], name=f'con_capacity_add[{s},{i}]')
    ...
    def spip_build(self, env, gp_env, K, tau, k_new = None, x_val = None, y_val = None, y_s_val = None, TL = None, ngc_list_tmp = None):
        ...
        obj_expr = gp.quicksum(env.cost1[i]*x[i] for i in range(env.r)) + \
            gp.quicksum(gp.quicksum((gp.quicksum(env.cost2[s,i][j] * y[k][i,j] for i in range(env.r) for j in range(env.N))+
                                     env.h * gp.quicksum(gp.quicksum(gp.quicksum(y[k][i,j]*y[k][i,l] for l in range(env.N) if l>j) for j in range(env.N)) for i in range(env.r)))*env.prob[s] for s in tau[k]) for k in range(K) if tau[k])+\
            gp.quicksum((gp.quicksum(env.cost2[s,i][j] * y_s[s][i,j] for i in range(env.r) for j in range(env.N))+
                         env.h * gp.quicksum(gp.quicksum(gp.quicksum(y_s[s][i,j]*y_s[s][i,l] for l in range(env.N) if l>j) for j in range(env.N)) for i in range(env.r)))*env.prob[s] for s in unseen_scen)
            ...
            if len(tau[k]) :
                for s in tau[k] :
                    for i in range(env.r) :
                        ...
                        con_capacity_add[k,s,i] = spip.addConstr(gp.quicksum(env.dem[s,i][j]*y[k][i,j] for j in range(env.N)) >= 0.80*env.V[i]*x[i], name=f'con_capacity_add[{k},{s},{i}]') 
        ...
        for s in unseen_scen :
            ...
            for i in range(env.r) :
                ...
                con_capacity_add_s[s,i] = spip.addConstr(gp.quicksum(env.dem[s,i][j]*y_s[s][i,j] for j in range(env.N)) >= 0.80*env.V[i]*x[i], name=f'con_capacity_add_s[{s},{i}]')
        ...
        if not(spip.Status == gp.GRB.OPTIMAL):
            ...
            for s in unseen_scen :
                ...
                obj_s = (sum(env.cost2[s,i][j] * y_s_sol[s][i,j] for i in range(env.r) for j in range(env.N)) +
                                           env.h * sum(sum(sum(y_s_sol[s][i,j]*y_s_sol[s][i,l] for l in range(env.N) if l>j) for j in range(env.N)) for i in range(env.r)))* env.prob[s]
                if obj_s > max_ob_s : 
                    max_ob_s = obj_s
                    s_max = s
    ...
    def spip_update(self, env, spip, K, tau, k_new, s_new, x_val, y_val, y_s_val, leaf = False, skip = False, TL = None):
        ...
        for i in range(env.r) :
            ...
            con_capacity_add[k_new,s_new,i] = spip.addConstr(gp.quicksum(env.dem[s_new,i][j]*y[k_new][i,j] for j in range(env.N)) >= 0.80*env.V[i]*x[i], name=f'con_capacity_add[{k_new},{s_new},{i}]')
        ...
        for i in range(env.r):
            ...
            temp_con_to_remove_s.append(spip.getConstrByName(f"con_capacity_add_s[{s_new},{i}]"))
        ...
        if skip :
            ...
            obj_val = obj_val_R + sum(sum((sum(env.cost2[s,i][j] * y_val[k][i,j] for i in range(env.r) for j in range(env.N)) +
                                           env.h * sum(sum(sum(y_val[k][i,j]*y_val[k][i,l] for l in range(env.N) if l>j) for j in range(env.N)) for i in range(env.r)))* env.prob[s]
                                                    for s in tau[k]) for k in range(K) if tau[k])
            # scenario selection
            max_ob_s = - np.inf
            s_max = env.N_scen
            for s in unseen_scen :
                obj_s = (sum(env.cost2[s,i][j] * y_s_val[s][i,j] for i in range(env.r) for j in range(env.N)) +
                                           env.h * sum(sum(sum(y_s_val[s][i,j]*y_s_val[s][i,l] for l in range(env.N) if l>j) for j in range(env.N)) for i in range(env.r)))* env.prob[s]
                obj_val += obj_s
                if obj_s > max_ob_s :
                    max_ob_s = obj_s
                    s_max = s
            ...
            if leaf :
                if k_new is not None :
                    ...
                    for i in range(env.r) :
                        ...
                        spip.addConstr(gp.quicksum(env.dem[s_new,i][j]*y_s[s_new][i,j] for j in range(env.N)) >= 0.80*env.V[i]*x[i], name=f'con_capacity_add_s[{s_new},{i}]') 

                    # remove constraints :
                    for i in range(env.r) :
                        spip.remove(con_capacity[k_new,s_new,i])
                        spip.remove(con_capacity_add[k_new,s_new,i])
    ...
    def sip(self, env, K, x, y, s):
        Klist = [k for k in range(K) 
                    ...
                    and  all(sum(env.dem[s,i][j]*y[k][i,j] for j in range(env.N)) >= 0.80* env.V[i]*x[i] for i in range(env.r))]
        ...
        if Klist == [] : 
            ...
            j = K 
            for k in Klist :
                obk = (sum(env.cost2[s,i][j] *y[k][i,j] for i in range(env.r) for j in range(env.N))+
                                     env.h * sum(sum(sum(y[k][i,j]*y[k][i,l] for l in range(env.N) if l>j) for j in range(env.N)) for i in range(env.r))) * env.prob[s]
                if obk < best_ob : 
                    best_ob = obk
                    j = k
