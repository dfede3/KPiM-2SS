...
class qtskp_Inst(problem_functions.Pb_functions):
    def __init__(self, tmax):
        self.tmax = tmax 

    def complete(self, env, gp_env) :
        
        model = gp.Model("Complete two-stage stochastic quadratic knapsack model", env = gp_env)
        # model parameters:
        ...
        # objective function
        obj_expr = - gp.quicksum(env.c[h]*x[h] for h in range(env.r)) + gp.quicksum(gp.quicksum(env.cost[s][i] * y[s][i] for i in range(env.N))*env.prob[s] +
                                                                                     gp.quicksum(gp.quicksum(env.qcost[s,i][j]*y[s][i]*y[s][j] for j in range(i+1,env.N)) for i in range(env.N))*env.prob[s] for s in range(env.N_scen)) 
        ...
        model.addConstrs(gp.quicksum(env.d[h]*x[h] for h in range(env.r)) + gp.quicksum(y[s][i]*env.weight[s][i] for i in range(env.N)) >= env.min_capacity[s] for s in range(env.N_scen))
    ...
    def spip_build(self, env, gp_env, K, tau, k_new = None, x_val = None, y_val = None, y_s_val = None, TL = None, ngc_list_tmp = None):
        ...
        obj_expr = - gp.quicksum(env.c[h]*x[h] for h in range(env.r)) + \
                        gp.quicksum(gp.quicksum(gp.quicksum(env.cost[s][i] * y[k][i] for i in range(env.N)) * env.prob[s] +
                                                gp.quicksum(gp.quicksum(env.qcost[s,i][j]*y[k][i]*y[k][j] for j in range(i+1,env.N)) for i in range(env.N))*env.prob[s]
                                                for s in tau[k]) for k in range(K) if tau[k]) + \
                        gp.quicksum(gp.quicksum(env.cost[s][i] * y_s[s][i] for i in range(env.N)) * env.prob[s] + 
                                    gp.quicksum(gp.quicksum(env.qcost[s,i][j]*y_s[s][i]*y_s[s][j] for j in range(i+1,env.N)) for i in range(env.N))*env.prob[s] for s in unseen_scen)
        ...
        for k in range(K):
            ...
            if len(tau[k]) :
                for s in tau[k] : 
                    ...
                    con_capacity_add[k,s] = spip.addConstr(gp.quicksum(env.d[h]*x[h] for h in range(env.r)) +
                                                       gp.quicksum(y[k][i]*env.weight[s][i] for i in range(env.N)) >= env.min_capacity[s], name = f"con_capacity_add[{k},{s}]" )
        con_capacity_s = {}
        con_capacity_add_s = {}
        con_x_y_s = {}
        for s in unseen_scen :
            ...
            con_capacity_add_s[s] = spip.addConstr(gp.quicksum(env.d[h]*x[h] for h in range(env.r)) +
                                                       gp.quicksum(y_s[s][i]*env.weight[s][i] for i in range(env.N)) >= env.min_capacity[s], name = f"con_capacity_add_s[{s}]" )
            for h in range(env.r):
        ...
        if not(spip.Status == gp.GRB.OPTIMAL):
            ...
            for s in unseen_scen : 
                ...
                obj_s = sum(env.cost[s][i] * y_s_sol[s][i] for i in range(env.N)) * env.prob[s] + sum(sum(env.qcost[s,i][j]*y_s_sol[s][i]*y_s_sol[s][j] for j in range(i+1,env.N)) for i in range(env.N))*env.prob[s]
                if obj_s > max_ob_s : 
                    max_ob_s = obj_s
                    s_max = s
    ...
    def spip_update(self, env, spip, K, tau, k_new, s_new, x_val, y_val, y_s_val, leaf = False, skip = False, TL = None):
        ...
        con_capacity_add[k_new,s_new] = spip.addConstr(gp.quicksum(env.d[h]*x[h] for h in range(env.r)) + gp.quicksum(y[k_new][i]*env.weight[s_new][i] for i in range(env.N)) >= env.min_capacity[s_new], name = f"con_capacity_add[{k_new},{s_new}]")
        
        temp_con_to_remove_s = []
        temp_con_to_remove_s.append(spip.getConstrByName(f"con_capacity_s[{s_new}]"))
        temp_con_to_remove_s.append(spip.getConstrByName(f"con_capacity_add_s[{s_new}]"))
        ...
        if skip :
            ...
            obj_val = obj_val_R - sum(sum(sum(env.cost[s][i] * y_val[k][i] for i in range(env.N)) * env.prob[s]
                                                                               + sum(sum(env.qcost[s,i][j]*y_val[k][i]*y_val[k][j] for j in range(i+1,env.N)) for i in range(env.N))*env.prob[s]
                                                    for s in tau[k]) for k in range(K) if tau[k])
            # scenario selection
            max_ob_s = - np.inf
            s_max = env.N_scen
            for s in unseen_scen :
                obj_s = sum(env.cost[s][i] * y_s_val[s][i] for i in range(env.N)) * env.prob[s] + sum(sum(env.qcost[s,i][j]*y_s_val[s][i]*y_s_val[s][j] for j in range(i+1,env.N)) for i in range(env.N))*env.prob[s]
                obj_val -= obj_s
                if obj_s > max_ob_s :
                    max_ob_s = obj_s
                    s_max = s
            ...
            if leaf :
                if k_new is not None :
                    ...
                    spip.addConstr(gp.quicksum(env.d[h]*x[h] for h in range(env.r)) +
                                                        gp.quicksum(y_s[s_new][i]*env.weight[s_new][i] for i in range(env.N)) >= env.min_capacity[s_new], name = f"con_capacity_add_s[{s_new}]" )
                    for h in range(env.r):
                        for i in range(env.q) :
                    ...
                    spip.remove(con_capacity_add[k_new,s_new])
    ...
    def sip(self, env, K, x, y, s):
        Klist = [k for k in range(K) 
                    ...
                    and sum(env.d[h]*x[h] for h in range(env.r)) + sum(y[k][i] * env.weight[s][i] for i in range(env.N)) >= env.min_capacity[s]]
